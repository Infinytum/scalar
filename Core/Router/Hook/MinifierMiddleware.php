...
class MinifierMiddleware implements HttpMiddlewareInterface
{
    ...
    private function fn_minify_html($input, $comment = 2, $quote = 1)
    {
        ...
        $output = $prev = "";
        foreach ($this->fn_minify([self::MINIFY_COMMENT_HTML, self::MINIFY_HTML_KEEP, self::MINIFY_HTML, self::MINIFY_HTML_ENT], $input) as $part) {
            if ($part === "\n") continue;
    ...
    private function fn_minify_js($input, $comment = 2)
    {
        if (!is_string($input) || !$input = $this->n(trim($input))) return $input;
        $output = $prev = "";
